 ...
 * de match d'un défi
 */
class Notif_feuille_completee_Defi extends React.Component {
        ...
        if(this.state.equipeUser != undefined) {
            return(
                <View style = {{justifyContent : "center", paddingTop : hp('1%')}}>
                    <Image
                    source = {{uri : this.state.equipeUser.photo}} 
        ...
        if(! this.state.isLoading) {
            return(
                <View style = {{flexDirection : 'row', marginTop : hp('2%')}}>
                    ...
                    <View>
                        <Text>Le capitaine de ton équipe {this.renderNomEquipe()} </Text>
                        <Text>a renseigné la feuille de match du défi contre</Text>
                        <Text>{this.renderNomAutreEquipe()}</Text>
                        <TouchableOpacity
...
export default withNavigation (Notif_feuille_completee_Defi)